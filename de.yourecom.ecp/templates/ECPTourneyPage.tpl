	...
	<div class="border">
		{if $tourneyList|isset && $tourneyList|count}
			<table class="tableList membersList">
				<thead>
					<tr class="tableHead">
						<th class="columnName{if $sortField == 'name'} active{/if}"><a href="index.php?page=ECPTourney&amp;sortField=name&amp;amp;sortOrder={if $sortField == 'name' && $sortOrder == 'ASC'}DESC{else}ASC{/if}&amp;packageID={@PACKAGE_ID}{@SID_ARG_2ND}">{lang}wcf.ecp.tourney.show.name{/lang}{if $sortField == 'name'} <img src="{icon}sort{@$sortOrder}S.png{/icon}" alt="" />{/if}</a></th>
						<th class="columnName{if $sortField == 'time'} active{/if}"><a href="index.php?page=ECPTourney&amp;sortField=time&amp;amp;sortOrder={if $sortField == 'time' && $sortOrder == 'ASC'}DESC{else}ASC{/if}&amp;packageID={@PACKAGE_ID}{@SID_ARG_2ND}">{lang}wcf.ecp.tourney.show.time{/lang}{if $sortField == 'time'} <img src="{icon}sort{@$sortOrder}S.png{/icon}" alt="" />{/if}</a></th>
						<th class="columnName{if $sortField == 'art'} active{/if}"><a href="index.php?page=ECPTourney&amp;sortField=art&amp;amp;sortOrder={if $sortField == 'art' && $sortOrder == 'ASC'}DESC{else}ASC{/if}&amp;packageID={@PACKAGE_ID}{@SID_ARG_2ND}">{lang}wcf.ecp.tourney.show.art{/lang}{if $sortField == 'art'} <img src="{icon}sort{@$sortOrder}S.png{/icon}" alt="" />{/if}</a></th>
						<th class="columnName{if $sortField == 'contacts'} active{/if}"><a href="index.php?page=ECPTourney&amp;sortField=contacts&amp;amp;sortOrder={if $sortField == 'contacts' && $sortOrder == 'ASC'}DESC{else}ASC{/if}&amp;packageID={@PACKAGE_ID}{@SID_ARG_2ND}">{lang}wcf.ecp.tourney.show.admin{/lang}{if $sortField == 'contacts'} <img src="{icon}sort{@$sortOrder}S.png{/icon}" alt="" />{/if}</a></th>
					</tr>
				</thead>
				<tbody>
