...
{* thread.tpl start *}
		...
		<script type="text/javascript">
			...
			onloadEvents.push(function() {
				if (document.referrer) {
					var postForm = document.referrer.search(/CliquePostEdit|CliquePostAdd/);
					if (postForm != -1) {
	...
	<script type="text/javascript">
		...
		var url = 'index.php?page=CliqueThread&cliqueID={$clique->cliqueID}&threadID={@$thread->threadID}&pageNo={@$pageNo}{@SID_ARG_2ND_NOT_ENCODED}';
		//]]>
	</script>
	{if $thread->canReplyThread() && $cliquePermissions->getCliquePermission('canMakePosts')}
		{assign var=cliqueID value=$clique->cliqueID}
		{include file='multiQuote' formURL="index.php?form=CliquePostAdd&cliqueID=$cliqueID&threadID=$threadID"|concat:SID_ARG_2ND_NOT_ENCODED}
	{else}
...
<div id="main">
    {include file="cliqueNavigation" showBoard=true}
	...
	<div class="mainHeadline">
		...
		<div class="headlineContainer">
			<h2 id="threadTitle{@$thread->threadID}">
				...
				<a href="index.php?page=CliqueThread&amp;cliqueID={$clique->cliqueID}&amp;threadID={@$thread->threadID}{@SID_ARG_2ND}">{$thread->topic}</a>
			</h2>
		</div>
	</div>
	
	<div class="contentHeader">
		{assign var=multiplePagesLink value="index.php?page=CliqueThread&threadID=$threadID&cliqueID=$cliqueID&pageNo=%d"}
		{if $highlight}
		...
		<div class="largeButtons">
			{if ($thread->canReplyThread() && $cliquePermissions->getCliquePermission('canMakePosts')) || $additionalLargeButtons|isset}
				<ul>
					{if $thread->canReplyThread() && $cliquePermissions->getCliquePermission('canMakePosts')}<li><a href="index.php?form=CliquePostAdd&amp;cliqueID={$clique->cliqueID}&amp;threadID={@$thread->threadID}{@SID_ARG_2ND}" id="replyButton1" title="{lang}wbb.thread.button.reply{/lang}"><img src="{icon}messageAddM.png{/icon}" alt="" /> <span>{lang}wbb.thread.button.reply{/lang}</span></a></li>{/if}
						...
						<div class="messageContentInner color-{cycle name=messageCycle}">
							<div class="messageHeader">
								<p class="messageCount">
									<a href="index.php?page=CliqueThread&amp;cliqueID={$clique->cliqueID}&amp;postID={@$post->postID}#post{@$post->postID}{@SID_ARG_2ND}" title="{lang}wbb.thread.permalink{/lang}" class="messageNumber">{#$startIndex}</a>
							...
							<div class="{@$messageFooterClass}">
								<div class="smallButtons">
									<ul id="postButtons{@$post->postID}">
										...
										{if $thread->canReplyThread() && $cliquePermissions->getCliquePermission('canMakePosts')}
											<li><a id="postQuote{@$post->postID}" href="index.php?form=CliquePostAdd&amp;cliqueID={$clique->cliqueID}&amp;postID={@$post->postID}&amp;action=quote{@SID_ARG_2ND}" title="{lang}wcf.multiQuote.button.quote{/lang}"><img src="{icon}messageQuoteS.png{/icon}" alt="" /> <span>{lang}wcf.multiQuote.button.quote{/lang}</span></a></li>
										{/if}
										{if $post->canEditPost($board, $thread)}
											<li><a href="index.php?form=CliquePostEdit&amp;cliqueID={$clique->cliqueID}&amp;postID={@$post->postID}{@SID_ARG_2ND}" title="{lang}wbb.thread.post.button.edit{/lang}"><img src="{icon}editS.png{/icon}" alt="" /> <span>{lang}wbb.thread.post.button.edit{/lang}</span></a></li>
										{/if}
										{if $this->user->userID}
											<li><a href="index.php?form=CliquePostReport&amp;cliqueID={$clique->cliqueID}&amp;postID={@$post->postID}&amp;postID={@$post->postID}{@SID_ARG_2ND}" title="{lang}wbb.thread.post.button.report{/lang}"><img src="{icon}postReportS.png{/icon}" alt="" /> <span>{lang}wbb.thread.post.button.report{/lang}</span></a></li>
										{/if}
	...
	{if THREAD_ENABLE_QUICK_REPLY == 1 && $thread->canReplyThread() && $cliquePermissions->getCliquePermission('canMakePosts') && $this->user->userID != 0}
		{include file='cliqueThreadQuickReply'}
	{/if}
	...
	<div class="contentFooter">
		...
		<div class="largeButtons">
			{if $thread->canReplyThread() || $additionalLargeButtons|isset || $cliquePermissions->getCliquePermission('canMakePosts')}
				<ul>
					{if $thread->canReplyThread() && $cliquePermissions->getCliquePermission('canMakePosts')}<li><a href="index.php?form=CliquePostAdd&amp;cliqueID={$clique->cliqueID}&amp;threadID={@$thread->threadID}{@SID_ARG_2ND}" id="replyButton2" title="{lang}wbb.thread.button.reply{/lang}"><img src="{icon}messageAddM.png{/icon}" alt="" /> <span>{lang}wbb.thread.button.reply{/lang}</span></a></li>{/if}
					{if $additionalLargeButtons|isset}{@$additionalLargeButtons}{/if}
				</ul>
			{/if}
		</div>
							...
							{foreach from=$similarThreads item=similarThread}
								<li{if BOARD_THREADS_ENABLE_MESSAGE_PREVIEW} title="{$similarThread->firstPostPreview}"{/if}>
									<ul class="breadCrumbs">
										<li><a href="index.php?page=CliqueBoard&amp;cliqueID={$clique->cliqueID}&amp;boardID={@$similarThread->boardID}{@SID_ARG_2ND}"><img src="{icon}boardS.png{/icon}" alt="" /> <span>{lang}{$similarThread->title}{/lang}</span></a> &raquo;</li>
										<li>
											<a href="index.php?page=CliqueThread&amp;cliqueID={$clique->cliqueID}&amp;threadID={@$similarThread->threadID}{@SID_ARG_2ND}"><img src="{icon}threadS.png{/icon}" alt="" /> {if $similarThread->prefix}<span class="prefix"><strong>{lang}{$similarThread->prefix}{/lang}</strong></span> {/if}<span>{$similarThread->topic}</span></a>
