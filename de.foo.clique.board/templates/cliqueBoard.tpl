...
<div id="main">
	...
	{* board.tpl start *}
	{include file="cliqueBoardList"}

	{if $board->isBoard()}
		<div class="contentHeader">
			{assign var=encodedPrefix value=$prefix|urlencode}
			{assign var=multiplePagesLink value="index.php?page=CliqueBoard&cliqueID=$cliqueID&boardID=$boardID&pageNo=%d"}
			...
			{if $board->canStartThread() || $additionalLargeButtons|isset}
				<div class="largeButtons">
					<ul>
						{if $board->canStartThread() && ($this->user->getPermission('mod.clique.general.cannStartPostsAllBoards') || $cliquePermissions->getCliquePermission('canMakePosts'))}<li><a href="index.php?form=CliqueThreadAdd&amp;cliqueID={$clique->cliqueID}&amp;boardID={@$boardID}{@SID_ARG_2ND}" title="{lang}wbb.board.button.newThread{/lang}"><img src="{icon}threadNewM.png{/icon}" alt="" /> <span>{lang}wbb.board.button.newThread{/lang}</span></a></li>{/if}
		...
		{if $permissions.canHandleThread || $permissions.canHandlePost || $cliquePermissions->getCliquePermission('canEditPosts')}
			<script type="text/javascript">
				//<![CDATA[
				var language = new Object();
				var postData = new Hash();
				var url = 'index.php?page=CliqueBoard&cliqueID={$clique->cliqueID}&boardID={@$boardID}&pageNo={@$pageNo}&sortField={@$sortField}&sortOrder={@$sortOrder}&daysPrune={@$daysPrune}&status={@$status}&prefix={@$prefix|encodeJS}&languageID={@$languageID}{@SID_ARG_2ND_NOT_ENCODED}';
				//]]>
			</script>
		...
		{else}
			...
			{if $topThreads|count > 0}
				{include file="cliqueBoardThreads" title="{lang}wbb.board.threads.top{/lang}" threads=$topThreads listName=topThreadsStatus listStatus=$topThreadsStatus listHasNewThreads=$newTopThreads}
			{/if}

			{if $normalThreads|count > 0}
				{include file="cliqueBoardThreads" title="{lang}wbb.board.threads.normal{/lang}" threads=$normalThreads listName=normalThreadsStatus listStatus=$normalThreadsStatus listHasNewThreads=$newNormalThreads}
			{/if}
		{/if}
					...
					<div class="containerContent">
						...
						<form method="get" action="index.php">
							<div class="threadSort">
								<input type="hidden" name="page" value="CliqueBoard" />
								...
								<div class="floatedElement">
									...
									<select name="status" id="filterByStatus">
										...
										{if $board->getModeratorPermission('canDeleteThreadCompletely') && $cliquePermissions->getCliquePermission('canDeletePosts')}<option value="deleted"{if $status == 'deleted'} selected="selected"{/if}>{lang}wbb.board.filterByStatus.deleted{/lang}</option>{/if}
					...
					<div class="containerContent">
						...
						<ul class="tagCloud">
							{foreach from=$tags item=tag}
								<li><a href="index.php?page=CliqueBoard&amp;cliqueID={$clique->cliqueID}&amp;boardID={@$board->boardID}&amp;pageNo={@$pageNo}&amp;sortField={@$sortField}&amp;sortOrder={@$sortOrder}&amp;daysPrune={@$daysPrune}&amp;status={@$status}&amp;prefix={@$encodedPrefix}&amp;languageID={@$languageID}&amp;tagID={@$tag->getID()}{@SID_ARG_2ND}" style="font-size: {@$tag->getSize()}%">{$tag->getName()}</a></li>
							{/foreach}
						</ul>
					</div>
				</div>
