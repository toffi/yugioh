...
class ECPTourneyRankingBFPage extends AbstractPage {
	public $templateName = 'ECPTourneyRankingBF';
	...
	public function readData() {
		...
		$sql = "SELECT events.id, events.officialEvent, COUNT(events_paarungen.userID1) AS userID1,
							...
							WHERE events.art = '3' && events_paarungen.event_round = '1' && events_paarungen.userID1 != '0' && 
								events_paarungen.userID2 != '0' && events.status = '10' && events.officialEvent = '3' &&
			...
			foreach ($this->tourneyList as $tourneyList) {
				...
				while ($row = WCF::getDB()->fetchArray()) {
					...
					if(!isset($fourth) && isset($third) && $row['userID1'] == $third) $fourth = $row['userID2'];
						...
						// Big Fight
				if($tourneyList['officialEvent'] == 3) {
					if($userList['user_ID'] == $first) $userList['points'] += 100;
					elseif($userList['user_ID'] == $second) $userList['points'] += 60;
					elseif($userList['user_ID'] == $third) $userList['points'] += 30;
					elseif($userList['user_ID'] == $fourth) $userList['points'] += 10;
				}
