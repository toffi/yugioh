...
class ECPTourneyRankingPage extends AbstractPage {
	public $templateName = 'ECPTourneyRanking';
	...
	public function readData() {
		parent::readData();
			...
			// Hole die Turniere
			$sql = "SELECT events.id, events.officialEvent, events.contacts, COUNT(events_paarungen.userID1) AS userID1,
							...
							WHERE events.art = '3' && events_paarungen.event_round = '1' && events_paarungen.userID1 != '0' && 
								events_paarungen.userID2 != '0' && events.status = '10'
							GROUP BY events.id";
			...
			while ($row = WCF::getDB()->fetchArray()) {
				...
				$tourneyList2['contacts'] = $row['contacts'];
			...
			foreach ($this->userList2 as $userList) {
				foreach ($this->tourneyList as $tourneyList) {
						// CreatorPints +1
					if($tourneyList['contacts'] == $userList['user_ID']) $userList['points']++;
				...
				// Matchpoints + ACP
					...
					while ($row = WCF::getDB()->fetchArray()) {
						...
						if(!isset($fourth) && isset($third) && $row['userID1'] == $third) $fourth = $row['userID2'];
							...
							// MNW
					if($tourneyList['officialEvent'] == 1) {
						if($userList['user_ID'] == $first) $userList['points'] += 25;
						elseif($userList['user_ID'] == $second) $userList['points'] += 15;
						elseif($userList['user_ID'] == $third) $userList['points'] += 5;
					}
							// WDFT
					elseif($tourneyList['officialEvent'] == 2) {
						if($userList['user_ID'] == $first) $userList['points'] += 10;
						elseif($userList['user_ID'] == $second) $userList['points'] += 6;
						elseif($userList['user_ID'] == $third) $userList['points'] += 3;
					}
							// Big Fight
					elseif($tourneyList['officialEvent'] == 3) {
						if($userList['user_ID'] == $first) $userList['points'] += 50;
						elseif($userList['user_ID'] == $second) $userList['points'] += 30;
						elseif($userList['user_ID'] == $third) $userList['points'] += 15;
						elseif($userList['user_ID'] == $fourth) $userList['points'] += 5;
					}
						// Kein offizielles FF Turnier
					else {
						if($tourneyList['number'] >= 8 && $tourneyList['number'] < 16) {
							if($userList['user_ID'] == $first) $userList['points'] += 8;
							elseif($userList['user_ID'] == $second) $userList['points'] += 6;
							elseif($userList['user_ID'] == $third) $userList['points'] += 3;
						}
						elseif($tourneyList['number'] >= 16 && $tourneyList['number'] < 32) {
							if($userList['user_ID'] == $first) $userList['points'] += 20;
							elseif($userList['user_ID'] == $second) $userList['points'] += 12;
							elseif($userList['user_ID'] == $third) $userList['points'] += 6;
						}
						elseif($tourneyList['number'] >= 32 && $tourneyList['number'] < 64) {
							if($userList['user_ID'] == $first) $userList['points'] += 30;
							elseif($userList['user_ID'] == $second) $userList['points'] += 18;
							elseif($userList['user_ID'] == $third) $userList['points'] += 9;
						}
						elseif($tourneyList['number'] >= 64) {
							if($userList['user_ID'] == $first) $userList['points'] += 40;
							elseif($userList['user_ID'] == $second) $userList['points'] += 24;
							elseif($userList['user_ID'] == $third) $userList['points'] += 12;
						}
					}
					unset($first);
					unset($second);
