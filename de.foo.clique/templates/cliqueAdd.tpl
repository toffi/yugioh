...
<head>
	<title>{lang}wcf.clique.add{/lang} - {lang}{PAGE_TITLE}{/lang}</title>
...
{include file='header' sandbox=false}
	...
	<div id="main">
		...
		<div class="mainHeadline">
				<img src="{icon}addM.png{/icon}" alt="" />
			<div class="headlineContainer">
				<h2>{lang}wcf.clique.add{/lang}</h2>
			</div>
		</div>
		...
		{/if}
			...
			<form method="post" action="index.php?form=CliqueAdd{SID_ARG_2ND_NOT_ENCODED}" enctype="multipart/form-data">
				<fieldset>
					...
					<div class="formElement{if $errorField == 'status'} formError{/if}">
						<div class="formFieldLabel">
							<label for="status">{lang}wcf.clique.add.status{/lang}</label>
						</div>
						<div class="formField">
							{foreach from=$statusArray item=status key=key}
								<input name="status" type="radio" id="status{$key}" value="{$key}"{if $status == $key} checked="checked"{/if} /> {$status}<br />
							{/foreach}
				...
				<fieldset>
					...
					<div class="formElement{if $errorField == 'pictureURL'} formError{/if}" id="pictureURLDiv">
						...
						<div class="formFieldDesc">{lang}wcf.clique.add.picture.download.description{/lang}</div >
