		...
		<thead>
			<tr class="tableHead">
				...
				<th colspan="2" class="columnTopic{if $sortField == 'topic'} active{/if}">
					<div><a href="index.php?page=CliqueBoard&amp;cliqueID={$clique->cliqueID}&amp;boardID={@$board->boardID}&amp;pageNo={@$pageNo}&amp;sortField=topic&amp;sortOrder={if $sortField == 'topic' && $sortOrder == 'DESC'}ASC{else}DESC{/if}&amp;daysPrune={@$daysPrune}&amp;status={@$status}&amp;prefix={@$encodedPrefix}&amp;languageID={@$languageID}&amp;tagID={@$tagID}{@SID_ARG_2ND}">
				...
				<th class="columnReplies{if $sortField == 'replies'} active{/if}">
					<div><a href="index.php?page=CliqueBoard&amp;cliqueID={$clique->cliqueID}&amp;boardID={@$board->boardID}&amp;pageNo={@$pageNo}&amp;sortField=replies&amp;sortOrder={if $sortField == 'replies' && $sortOrder == 'DESC'}ASC{else}DESC{/if}&amp;daysPrune={@$daysPrune}&amp;status={@$status}&amp;prefix={@$encodedPrefix}&amp;languageID={@$languageID}&amp;tagID={@$tagID}{@SID_ARG_2ND}">
				...
				<th class="columnViews{if $sortField == 'views'} active{/if}">
					<div><a href="index.php?page=CliqueBoard&amp;cliqueID={$clique->cliqueID}&amp;boardID={@$board->boardID}&amp;pageNo={@$pageNo}&amp;sortField=views&amp;sortOrder={if $sortField == 'views' && $sortOrder == 'DESC'}ASC{else}DESC{/if}&amp;daysPrune={@$daysPrune}&amp;status={@$status}&amp;prefix={@$encodedPrefix}&amp;languageID={@$languageID}&amp;tagID={@$tagID}{@SID_ARG_2ND}">
				...
				<th class="columnLastPost{if $sortField == 'lastPostTime'} active{/if}">
					<div><a href="index.php?page=CliqueBoard&amp;cliqueID={$clique->cliqueID}&amp;boardID={@$board->boardID}&amp;pageNo={@$pageNo}&amp;sortField=lastPostTime&amp;sortOrder={if $sortField == 'lastPostTime' && $sortOrder == 'DESC'}ASC{else}DESC{/if}&amp;daysPrune={@$daysPrune}&amp;status={@$status}&amp;prefix={@$encodedPrefix}&amp;languageID={@$languageID}&amp;tagID={@$tagID}{@SID_ARG_2ND}">
					...
					<tr class="{cycle name='className'}" id="threadRow{@$thread->threadID}">
						...
						<td class="columnTopic"{if BOARD_THREADS_ENABLE_MESSAGE_PREVIEW && $board->getPermission('canReadThread')} title="{$thread->firstPostPreview}"{/if}>
							<div id="thread{@$thread->threadID}" class="topic{if $thread->isNew()} new{/if}{if $thread->ownPosts || $thread->subscribed} interesting{/if}">
								{if $thread->isNew()}
									<a id="gotoFirstNewPost{@$thread->threadID}" href="index.php?page=CliqueThread&amp;cliqueID={$clique->cliqueID}&amp;threadID={@$thread->threadID}&amp;action=firstNew{@SID_ARG_2ND}"><img class="goToNewPost" src="{icon}goToFirstNewPostS.png{/icon}" alt="" title="{lang}wbb.index.gotoFirstNewPost{/lang}" /></a>
								{/if}
							...
							<div class="statusDisplay">
								{if BOARD_THREADS_ENABLE_SMALL_PAGES}{smallpages pages=$thread->getPages($board) link="index.php?page=CliqueThread&cliqueID=$cliqueID&threadID=$threadID&pageNo=%d"|concat:SID_ARG_2ND_NOT_ENCODED}{/if}
						...
						<td class="columnLastPost">
							{if $thread->replies != 0}
								<div class="containerIconSmall">
									<a href="index.php?page=CliqueThread&amp;cliqueID={$clique->cliqueID}&amp;threadID={@$thread->threadID}&amp;action=lastPost{@SID_ARG_2ND}"><img src="{icon}goToLastPostS.png{/icon}" alt="" title="{lang}wbb.index.gotoLastPost{/lang}" /></a>
								</div>
