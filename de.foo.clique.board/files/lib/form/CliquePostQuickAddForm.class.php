...
// wbb imports
require_once(WBB_DIR.'lib/form/PostQuickAddForm.class.php');
 ...
 */
class CliquePostQuickAddForm extends PostQuickAddForm {
	// system
	public $templateName = 'cliquePostAdd';
    public $cliqueID = 0;
    public $isMember = 0;
	...
	public function save() {
		...
		if ($postID = PostEditor::test($this->subject, $this->text, WCF::getUser()->userID, $this->username, $this->threadID)) {
			HeaderUtil::redirect('index.php?page=CliqueThread&cliqueID='.$this->cliqueID.'&postID=' . $postID . SID_ARG_2ND_NOT_ENCODED . '#post' . $postID);
			exit;
		}
		parent::save();
	}
	...
	public function show() {
		if (!CliqueEditor::getCliquePermission('canMakePosts') && !WCF::getUser()->getPermission('mod.clique.general.cannSartPostsAllBoards')) {
